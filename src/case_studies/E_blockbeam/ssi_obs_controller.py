...
class BlockbeamSSIOController(common.ControllerBase):
    def __init__(self, separate_integrator=False):
        ...
        des_obs_poles = np.array([-110, -90, -95, -100])
        print(f"{des_obs_poles = }")
        
        self.u_fl = lambda x: np.array([1/2 * P.m2*P.g + P.m1*P.g* x[0] /P.ell])
        self.u_prev = np.zeros(P.B.shape[1])
        self.observer = Observer(P.A, P.B, P.Cm, des_obs_poles, P.ts, self.x_eq, self.u_eq, self.u_fl)

    def update_with_measurement(self, r, y):
        # update observer with measurement
        xhat = self.observer.update(y, self.u_prev)
        ...
        u_unsat = u_tilde + self.u_fl(xhat)
...
if __name__ == "__main__":
    controller = BlockbeamSSIOController()