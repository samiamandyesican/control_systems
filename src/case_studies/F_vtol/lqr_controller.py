...
class VTOLControllerSSIDO(common.ControllerBase):
    def __init__(self, separate_integrator=True):
        ...
        # tuning parameters
        # Q = 5 * np.diag([30.0, 30.0, 1.0, 0.15, 0.03, 0.2, 21.0, 21.0]) # [z_v, h, theta, z_vdot, hdot, thetadot, integrator_z, integrator_h]
        # R = np.diag([0.8, 3.0]) # [F, tau]
        Q = np.diag([10, 10, 1, 0.01, 0.01, 1.0, 40.0, 40.0]) # [z_v, h, theta, z_vdot, hdot, thetadot, integrator_z, integrator_h]
        R = np.diag([0.5, 1.0]) # [F, tau]
